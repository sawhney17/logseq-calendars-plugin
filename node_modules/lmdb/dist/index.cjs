...
let Env, Compression, Cursor, getAddress, setGlobalBuffer,
    require$2, arch, fs, lmdbError, path, EventEmitter, orderedBinary, MsgpackrEncoder, WeakLRUCache;
...
function addReadMethods(LMDBStore, {
	...
	Object.assign(LMDBStore.prototype, {
		getString(id) {
			...
			if (string)
				lastSize$1 = string.length;
			return string;
		},
		getBinaryFast(id) {
			(env.writeTxn || (readTxnRenewed ? readTxn : renewReadTxn()));
			try {
				lastSize$1 = this.db.getByBinary(this.writeKey(id, keyBytes, 0));
			} catch (error) {
			...
			if (lastSize$1 > bytes.maxLength) {
				if (lastSize$1 === 0xffffffff)
					return;
				bytes = this._allocateGetBuffer(lastSize$1);
				lastSize$1 = this.db.getByBinary(this.writeKey(id, keyBytes, 0));
			}
			bytes.length = lastSize$1;
			return bytes;
		},
		...
		getBinary(id) {
			...
			return fastBuffer && Uint8ArraySlice.call(fastBuffer, 0, lastSize$1);
		},
		get(id) {
			if (this.decoder) {
		...
		doesExist(key, versionOrValue) {
			...
			if (versionOrValue === undefined) {
				...
				return lastSize$1 !== 0xffffffff;
			}
			else if (this.useVersions) {
				this.getBinaryFast(key);
				return lastSize$1 !== 0xffffffff && getLastVersion() === versionOrValue;
			}
			else {
		...
		getSharedBufferForGet(id) {
			...
			lastSize$1 = this.keyIsCompatibility ? txn.getBinaryShared(id) : this.db.get(this.writeKey(id, keyBytes, 0));
			if (lastSize$1 === 0xffffffff) { // not found code
				return; //undefined
			}
			return lastSize$1;
		},
...
function open(path$1, options) {
	...
	class LMDBStore extends EventEmitter {
		constructor(dbName, dbOptions) {
			...
			if (dbOptions.compression instanceof Compression) ; else if (dbOptions.compression && typeof dbOptions.compression == 'object')
...
setExternals({
	require: require$1, arch: os.arch, fs: fs__default["default"], path: path__default["default"], MsgpackrEncoder: msgpackr.Encoder, WeakLRUCache: weakLruCache.WeakLRUCache, orderedBinary: orderedBinary__namespace, EventEmitter: EventEmitter__default["default"]
});
var nodeIndex = {
